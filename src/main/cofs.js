 ...
 */
cofs.readFile = function(path, options) {
 ...
 * @param {String | { encoding?: string, mode?: string | number, flag?: string }} [options] 
 */
cofs.writeFile = function(path, data, options) {
 ...
 * @returns {Promise<Boolean>}
 */
cofs.isDirectory = function(path) {
 ...
 */
cofs.readJson = function(filePath) {
 ...
 * @returns {Promise}
 */
cofs.writeJson = function(filePath, data, space) {
    return new Promise((resolve, reject) => {
 ...
 */
cofs.readDir = function(filePath) {
    return new Promise((resolve, reject) => {
 ...
 * @returns {Promise}
 */
cofs.rm = function rm(filePath, filter) {
 ...
 * @returns {Promise}
 */
cofs.cp = function cp(srcPath, src2dst, overwrite) {
 ...
 * @param {Promise}
 */
cofs.mkdirs = function mkdirs(dir) {
    var dirs = dir.split(/[\/\\]/);
    let tasks = [];
...
Promise.flow = function(tasks) {
    return new Promise((resolve, reject) => {
        ...
        function next(data) {
            ret.push(data);
            if (i < tasks.length)
                tasks[i++]().then(next, reject).catch(reject);
            else
                resolve(ret);
        }
        tasks[i++]().then(next, reject).catch(reject);
